...
class Main {
    ...
    public static String calc(String input) {
            if (!input.contains(" ")) {
        throw new IllegalArgumentException("Input must include spaces between operands and operators.");
    }
        String[] parts = input.split(" ");
        if (parts.length != 3) {
            throw new IllegalArgumentException("Invalid input only 2 operands are allowed");
        }
        ...
        if (parts[0].matches("\\d+") && parts[2].matches("\\d+")) {
            ...
            int num2 = Integer.parseInt(parts[2]);
              ...
              if (num1 < 1 || num1 > 10 || num2 < 0 || num2 > 10) {
                throw new IllegalArgumentException("Arabic numerals must represent numbers between 1 and 10");
            }
            ...
            switch (operator) {
                ...
                case "-":
                    result = num1 - num2;
                    if (result < 1) {
                        throw new IllegalArgumentException("Roman numbers doesn't have negative value");
                    }
                    break;
                case "*":
                    result = num1 * num2;
